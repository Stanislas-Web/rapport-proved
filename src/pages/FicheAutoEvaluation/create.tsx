...
import { CreateFicheAutoEvaluationRequest } from '../../models/FicheAutoEvaluation';
import DefaultLayout from '../../layout/DefaultLayout';
import Breadcrumb from '../../components/Breadcrumbs/Breadcrumb';
import toast from 'react-hot-toast';

const CreateFicheAutoEvaluationPage: React.FC = () => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<CreateFicheAutoEvaluationRequest>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Créer les données avec l'ID utilisateur et le statut "soumis"
      const ficheData = {
        ...formData,
      ...
      await ficheAutoEvaluationService.createFiche(ficheData);
      toast.success('Fiche d\'auto-évaluation créée avec succès');
      navigate('/fiche-auto-evaluation');
    } catch (error: any) {
      console.error('Erreur lors de la création:', error);
      
      // Vérifier si c'est une erreur d'authentification
      if (error.message === 'Utilisateur non authentifié' || error.status === 401) {
      ...
      if (error.message?.includes('temps') || error.message?.includes('connexion')) {
  ...
  return (
    <>
      {/* <Breadcrumb pageName="Créer une Fiche d'Auto-évaluation" /> */}
...
export default CreateFicheAutoEvaluationPage;
