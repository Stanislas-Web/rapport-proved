...
// Assuming getAllUser action exists
...
const TableEleve = () => {
  ...
  useEffect(() => {
    const fetchEcoles = async () => {
      ...
      if (email && password) {
        try {
          ...
          dispatch(setEcoleStore(result.data)); // Set ecoles in the store
        } catch (err) {
    ...
    fetchEcoles();
  }, [dispatch]);// Empty dependency array to avoid re-fetching
          ...
          <table className="w-full table-auto">
            <thead>
              <tr className="bg-gray-2 text-left dark:bg-meta-4">
                ...
                <th className="min-w-[250px] py-4 px-4 font-medium text-black dark:text-white xl:pl-11">Adresse</th>
                <th className="min-w-[250px] py-4 px-4 font-medium text-black dark:text-white">Sous-Proved</th>
            ...
            <tbody>
              {[...ecoles].reverse().map((ecolesItem, key) => (
                <tr key={key}>
                  <td className="border-b  border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                    {ecolesItem.nom}
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11 ">
                    <h5 className="font-medium text-black dark:text-white ">{ecolesItem.adresse}</h5>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                    <h5 className="font-medium text-black dark:text-white">{ecolesItem.sousDirection.nom}</h5>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                    <h5 className="font-medium text-black dark:text-white">{ecolesItem.createdBy.nom} {ecolesItem.createdBy.prenom}</h5>
                  </td>
                  {/* <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                    <h5 className="font-medium text-black dark:text-white">{ecolesItem.createdAt}</h5>
                  </td> */}
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p
                      className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-xs font-medium ${ true === true ? ' bg-orange-400 text-orange-400' : 'bg-success text-success '
                        }`}
                    >
                      {true === true ? 'Non validé' : 'Validé'}
                    </p>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
